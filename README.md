...
纯 CPU 文档解析服务。PDF 统一使用 OpenDataLoader + 按页 RapidOCR 管线；DOC 先由
LibreOffice 转为 DOCX，再使用 Mammoth 转语义 HTML，并收敛为 Markdown 产物。

## 能力

- `POST /v1/parse`：同步解析图片、PDF、DOC 或 DOCX
...
前置条件：Python 3.10/3.11、Java 11+、LibreOffice Writer、Redis、MySQL 8.0+。
...
DOCX 上传后会先验证 OOXML 容器、成员路径、压缩比和解压后资源上限。DOC 上传后先检查
OLE Compound File 文件头，再由 LibreOffice 在独立用户配置和临时目录中转换为 DOCX；转换产物
同样执行上述安全校验。之后统一执行：
...
Word 的最终产物固定为 `outputs.markdown`。请求中的 `output_formats` 只控制 PDF 和图片输出，
不会为 DOC 或 DOCX 额外生成 Text、JSON 或 HTML 产物。
...
Word 内嵌图片，RapidOCR 解析 PDF 时会导出页面渲染图，直接上传图片时会保存原图。Word 图片
...
扫描 PDF、混合 PDF，以及包含标题、列表、复杂表格、公式和图片的 DOC 与 DOCX。
